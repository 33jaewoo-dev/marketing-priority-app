...
def calc_score(inp, df_ref, ml_label=None):
    """
    Market Attractiveness Score (0-100)
    Economically grounded directional logic for every variable.
    Hybrid: Rule-based base score + ML adjustment layer.
    """
    v = inp['visitor_count']
    f_vol = (20 if v>500000 else 17 if v>300000 else 14 if v>200000 else
             11 if v>100000 else 8 if v>50000 else 5 if v>20000 else 2)

    mom = inp['visitor_mom_growth']
    f_mom = (20 if mom>0.30 else 17 if mom>0.20 else 14 if mom>0.10 else
             11 if mom>0.05 else 8 if mom>0.00 else 6 if mom>-0.05 else
             4 if mom>-0.15 else 2 if mom>-0.30 else 0)
    vvs3 = inp['visitor_vs_3m_avg']
    f_mom = min(20, f_mom + (3 if vvs3>1.30 else 2 if vvs3>1.15 else 1 if vvs3>1.05 else 0))

    e = inp['engagement']
    f_eng = (15 if e>3000000 else 13 if e>1000000 else 11 if e>500000 else
             9 if e>200000 else 6 if e>50000 else 3 if e>10000 else 1)

    b = inp['buzz_volume']
    f_buzz = (8 if b>300000 else 7 if b>100000 else 6 if b>50000 else
              5 if b>30000 else 3 if b>10000 else 2 if b>3000 else 1)

    pe = inp['potential_exposure']
    f_exp = (5 if pe>5000000 else 4 if pe>2000000 else 3 if pe>1000000 else
             2 if pe>500000 else 1 if pe>100000 else 0)

    sns_mom = inp['engagement_mom_growth']*0.65 + inp['buzz_mom_growth']*0.35
    f_sns_g = (10 if sns_mom>0.30 else 8 if sns_mom>0.20 else 7 if sns_mom>0.10 else
               5 if sns_mom>0.00 else 3 if sns_mom>-0.10 else 2 if sns_mom>-0.20 else 0)

    f_sent = inp['positive_pct'] * 0.10  # linear: every 1% = +0.1pt

    cs = inp['country_share']
    f_share = (5 if cs>0.40 else 4 if cs>0.30 else 3 if cs>0.20 else 2 if cs>0.10 else 1)

    h = inp['hallyu_spend_count']
    f_kwave = (7 if h>1500000 else 6 if h>1000000 else 5 if h>500000 else
               3 if h>200000 else 2 if h>50000 else 0)

    base = f_vol + f_mom + f_eng + f_buzz + f_exp + f_sns_g + f_sent + f_share + f_kwave

    # Visitor crash penalty
    p_shock = (-25 if mom<-0.50 else -15 if mom<-0.30 else -7 if mom<-0.15 else 0)

    # Negative sentiment penalty
    pos = inp['positive_pct']
    p_sent = (-15 if pos<40 else -7 if pos<50 else 0)

    # Oil price penalty: high oil → higher airfare → Korea less accessible
    oil = inp['oil_price']
    p_oil = (-14 if oil>130 else -9 if oil>110 else -5 if oil>90 else -2 if oil>70 else 0)

    # Weak KRW bonus: high exchange rate → Korea cheaper for foreign tourists
    exr = inp['exchange_rate']
    p25 = float(df_ref['exchange_rate'].quantile(0.25))
    p50 = float(df_ref['exchange_rate'].quantile(0.50))
    p75 = float(df_ref['exchange_rate'].quantile(0.75))
    p90 = float(df_ref['exchange_rate'].quantile(0.90))
    b_fx = (12 if exr>p90 else 8 if exr>p75 else 5 if exr>p50 else 2 if exr>p25 else 0)

    b_peak = 4 if inp['is_peak_season'] else 0
    b_syn  = (4 if mom>0.10 and sns_mom>0.10 else 2 if mom>0.05 and sns_mom>0.00 else 0)

    score = base + p_shock + p_sent + p_oil + b_fx + b_peak + b_syn

    # ML Hybrid Adjustment — applied across all score ranges
    ml_adj = 0; ml_note = ""
    if ml_label is not None:
        if ml_label == 'High':
            if   score < 40: ml_adj, ml_note = +15, "▲ ML: strong composite signal despite low rule score"
            elif score < 55: ml_adj, ml_note = +10, "▲ ML: above-average multi-factor interaction"
            elif score < 65: ml_adj, ml_note = +6,  "▲ ML: moderate upward correction"
            elif score < 75: ml_adj, ml_note = +3,  "▲ ML: slight upward nudge"
        elif ml_label == 'Low':
            if   score > 75: ml_adj, ml_note = -15, "▼ ML: hidden risk pattern despite high rule score"
            elif score > 60: ml_adj, ml_note = -10, "▼ ML: below-average composite signal"
            elif score > 50: ml_adj, ml_note = -6,  "▼ ML: moderate downward correction"
            elif score > 40: ml_adj, ml_note = -3,  "▼ ML: slight downward nudge"
        elif ml_label == 'Medium':
            if   score > 80: ml_adj, ml_note = -8,  "▼ ML: mixed signals temper very high rule score"
            elif score > 70: ml_adj, ml_note = -4,  "▼ ML: slight downward correction toward medium range"
            elif score < 30: ml_adj, ml_note = +8,  "▲ ML: partial positives lift very low rule score"
            elif score < 40: ml_adj, ml_note = +4,  "▲ ML: slight upward correction toward medium range"

    final = round(float(np.clip(score + ml_adj, 0, 100)), 1)
    breakdown = {
        'Visitor Volume':      f_vol,
        'Visitor Momentum':    f_mom,
        'SNS Engagement':      f_eng,
        'Buzz Volume':         f_buzz,
        'Potential Exposure':  f_exp,
        'SNS Momentum':        f_sns_g,
        'Sentiment (linear)':  round(f_sent,1),
        'Market Share':        f_share,
        'Korean Wave Spending':f_kwave,
        'Visitor Crash':       p_shock,
        'Sentiment Penalty':   p_sent,
        'Oil Price Penalty':   p_oil,
        'Weak KRW Bonus':      b_fx,
        'Peak Season':         b_peak,
        'Growth Synergy':      b_syn,
...
if page == "📊  Overview":
    ...
    # Recalculate scores using the same formula as Priority Engine
    latest_raw = df[df['year_month']==latest_month].copy()
    scores = []
    for _, row in latest_raw.iterrows():
        inp = {
            'visitor_count':        row.get('visitor_count', 0) or 0,
            'visitor_mom_growth':   row.get('visitor_mom_growth', 0) or 0,
            'visitor_vs_3m_avg':    row.get('visitor_vs_3m_avg', 1) or 1,
            'engagement':           row.get('engagement', 0) or 0,
            'buzz_volume':          row.get('buzz_volume', 0) or 0,
            'potential_exposure':   row.get('potential_exposure', 0) or 0,
            'engagement_mom_growth':row.get('engagement_mom_growth', 0) or 0,
            'buzz_mom_growth':      row.get('buzz_mom_growth', 0) or 0,
            'hallyu_spend_count':   row.get('hallyu_spend_count', 0) or 0,
            'is_peak_season':       row.get('is_peak_season', 0) or 0,
            'positive_pct':         row.get('positive_pct', 50) or 50,
            'country_share':        row.get('country_share', 0.2) or 0.2,
            'exchange_rate':        row.get('exchange_rate', 200) or 200,
            'oil_price':            row.get('oil_price', 80) or 80,
        }
        s, _ = calc_score(inp, df)
        scores.append(s)
    latest_raw['calc_score'] = scores
    latest = latest_raw.sort_values('calc_score', ascending=False)
    ...
    for i, (_, row) in enumerate(latest.iterrows()):
        ...
        ps   = float(row.get('calc_score', 0) or 0)
        with cols[i]:
            st.markdown(f"""
...
# ══════════════════════════════════════════════════════════════════
    ...
    if st.button("🔮  Run Priority Analysis", use_container_width=True):
        ...
        # Attractiveness Score (0-100) with ML adjustment
        p_score, breakdown = calc_score(inp, df, ml_label=lbl)
        ...
        with r1:
            ...
            bd_keys = ["Visitor Volume","Visitor Momentum","SNS Volume","SNS Momentum",
                       "Sentiment","Hallyu Spending","Shock Penalty","Sentiment Penalty","Peak Bonus","ML Adjustment"]
            bd_rows = ""
            for k in bd_keys:
                v = breakdown.get(k, 0)
                if v == 0 and k not in ["Visitor Volume","Visitor Momentum","SNS Volume","SNS Momentum","Sentiment"]:
                    continue
                clr_v = "#16a34a" if (isinstance(v,(int,float)) and v > 0) else "#dc2626" if (isinstance(v,(int,float)) and v < 0) else "#94a3b8"
                prefix = "+" if isinstance(v,(int,float)) and v > 0 else ""
                bd_rows += f'<div style="display:flex;justify-content:space-between;font-size:11px;padding:3px 0;"><span style="color:#64748b;">{k}</span><span style="font-weight:600;color:{clr_v};">{prefix}{v}</span></div>'
            ml_note_html = f'<div style="font-size:10px;color:#3b82f6;margin-top:6px;padding-top:6px;border-top:1px solid #e2e8f0;">{breakdown["ml_note"]}</div>' if breakdown.get("ml_note") else ""
            _breakdown_html = f'<div style="background:#f8fafc;border-radius:10px;padding:12px;text-align:left;margin-bottom:12px;"><div style="font-size:10px;color:#94a3b8;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">Score Breakdown</div>{bd_rows}{ml_note_html}</div>'
...
# ══════════════════════════════════════════════════════════════════
    ...
    with c2:
        MMAP={'Visitor Count':'visitor_count','MoM Growth':'visitor_mom_growth',
              ...
              'Korean Wave Transactions':'hallyu_spend_count','Priority Score':'priority_score'}

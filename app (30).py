...
# ── Action Plan lookup by Growth Driver ──────────────────────────
ACTION_PLANS = {
...
def calc_whatif_adjustment(v_chg, s_chg, bz_chg, se_chg, oi_chg, ex_chg):
    ...
    Uses smooth exponential curve: f(x) = cap * (1 - exp(-k * |x|))
    Fully continuous — every 1-unit change produces a different output.
    Not used for model training, evaluation, or main dashboard ranking.
    """
    def cont(chg, cap, k):
        if chg == 0: return 0.0
        sign = 1 if chg > 0 else -1
        return round(sign * cap * (1 - np.exp(-k * abs(chg))), 2)

    # Visitor count: strongest demand signal (cap=18, k=0.05)
    v_adj  = cont(v_chg,   18, 0.05)

    # Positive sentiment: direct attractiveness signal (cap=15, k=0.07)
    s_adj  = cont(se_chg,  15, 0.07)

    # SNS engagement: digital interest signal (cap=12, k=0.04)
    e_adj  = cont(s_chg,   12, 0.04)

    # Buzz volume: awareness signal (cap=10, k=0.04)
    bz_adj = cont(bz_chg,  10, 0.04)

    # Oil price: NEGATIVE direction — higher oil = lower score (cap=12, k=0.04)
    oi_adj = cont(-oi_chg, 12, 0.04)

    # Exchange rate: higher = weaker KRW = more affordable (cap=12, k=0.05)
    ex_adj = cont(ex_chg,  12, 0.05)

    total = round(float(np.clip(v_adj+s_adj+e_adj+bz_adj+oi_adj+ex_adj, -25, 25)), 2)
    return total, {
        'visitor':    round(float(v_adj),2),
        'sentiment':  round(float(s_adj),2),
        'engagement': round(float(e_adj),2),
        'buzz':       round(float(bz_adj),2),
        'oil':        round(float(oi_adj),2),
        'exchange':   round(float(ex_adj),2),
    }
...
# ══════════════════════════════════════════════════════════════════
    ...
    if st.button("🤖  Run ML Priority Prediction",use_container_width=True):
        ...
        # ── Single-variable monotonic sanity + minimum visible movement ──
        # When only one slider is active:
        # (a) guarantee directional consistency (no reversal)
        # (b) ensure a minimum visible movement so large changes feel responsive
        # This is a UI post-processing layer — NOT used in model training/evaluation.
        if is_whatif:
            active = sum(1 for x in [v_chg,s_chg,bz_chg,se_chg,oi_chg,ex_chg] if x!=0)
            if active == 1:
                def min_move(chg, adj, cap=2.0):
                    """Minimum visible movement = min(cap, abs(adj)*0.5), scaled by slider size."""
                    return min(cap, abs(adj) * 0.5) if chg != 0 else 0

                # Positive-direction variables
                if v_chg > 0:
                    floor = base_score + min_move(v_chg, adj_breakdown.get('visitor',0), 2.0)
                    raw_scenario = max(raw_scenario, floor)
                if se_chg > 0:
                    floor = base_score + min_move(se_chg, adj_breakdown.get('sentiment',0), 2.0)
                    raw_scenario = max(raw_scenario, floor)
                if s_chg > 0:
                    floor = base_score + min_move(s_chg, adj_breakdown.get('engagement',0), 1.5)
                    raw_scenario = max(raw_scenario, floor)
                if bz_chg > 0:
                    floor = base_score + min_move(bz_chg, adj_breakdown.get('buzz',0), 1.0)
                    raw_scenario = max(raw_scenario, floor)
                if ex_chg > 0:
                    floor = base_score + min_move(ex_chg, adj_breakdown.get('exchange',0), 1.5)
                    raw_scenario = max(raw_scenario, floor)

                # Negative-direction variables
                if v_chg < 0:
                    ceil_ = base_score - min_move(v_chg, adj_breakdown.get('visitor',0), 2.0)
                    raw_scenario = min(raw_scenario, ceil_)
                if se_chg < 0:
                    ceil_ = base_score - min_move(se_chg, adj_breakdown.get('sentiment',0), 2.0)
                    raw_scenario = min(raw_scenario, ceil_)
                if s_chg < 0:
                    ceil_ = base_score - min_move(s_chg, adj_breakdown.get('engagement',0), 1.5)
                    raw_scenario = min(raw_scenario, ceil_)
                if bz_chg < 0:
                    ceil_ = base_score - min_move(bz_chg, adj_breakdown.get('buzz',0), 1.0)
                    raw_scenario = min(raw_scenario, ceil_)
                if oi_chg > 0:   # oil up = score down
                    ceil_ = base_score - min_move(oi_chg, abs(adj_breakdown.get('oil',0)), 1.5)
                    raw_scenario = min(raw_scenario, ceil_)
                if oi_chg < 0:   # oil down = score up
                    floor = base_score + min_move(oi_chg, abs(adj_breakdown.get('oil',0)), 1.5)
                    raw_scenario = max(raw_scenario, floor)
                if ex_chg < 0:
                    ceil_ = base_score - min_move(ex_chg, adj_breakdown.get('exchange',0), 1.5)
                    raw_scenario = min(raw_scenario, ceil_)

        scenario_score=round(raw_scenario,1)
        pos_d,neg_d=get_market_signals(inp)
        mv=inp['visitor_count']*(spend or 0)
        rev_opp,rev_cls=get_revenue_opportunity(ml_score,mv,inp.get('visitor_mom_growth',0),inp.get('engagement',0),inp.get('hallyu_spend_count',0),inp.get('positive_pct',50),all_market_vals=_all_mv,all_kwave_vals=_all_kw)
        try:
        ...
        with r1:
            ...
            if is_whatif:
                ...
                adj_html = (
                    ...
                    f'Raw ML Score: {ml_score} &nbsp;·&nbsp; Directional adjustment: {adj_total:+.1f} pts</div>'
                    f'<div style="font-size:9px;color:#94a3b8;margin-top:2px;font-style:italic;">'
                    f'Adjustment applied only to scenario simulation — not used for model training or evaluation</div>'
                    f'<div style="margin-top:5px;">'
                    f'<span style="background:#f1f5f9;border-radius:6px;padding:2px 8px;font-size:10px;color:#475569;">Scenario Score Tier: {scenario_tier}</span>'
                    f'&nbsp;<span style="font-size:9px;color:#94a3b8;font-style:italic;">≠ ML class prediction</span></div>'
                )
            else:
                adj_html = ''
            _result_html = (
                ...
                f'<div style="font-size:10px;color:#94a3b8;letter-spacing:.1em;text-transform:uppercase;margin-bottom:4px;">{score_label}</div>'
                f'<div style="font-size:10px;color:#94a3b8;margin-bottom:4px;">'+('Raw ML Score + bounded scenario adjustment' if is_whatif else 'High Priority Probability × 100')+'</div>'
                ...
                f'<div style="font-size:14px;color:#94a3b8;margin-bottom:6px;">/ 100 · High prob: <b style="color:{clr};">{pd_["High"]*100:.0f}%</b></div>'
                + adj_html +
                f'<div style="margin-top:4px;">{raw_diff_html}</div>'
                f'</div>'
                f'<div style="background:#f1f5f9;border-radius:8px;height:10px;margin:12px 0 6px;overflow:hidden;">'
                ...
                f'<div style="font-size:10px;color:#94a3b8;margin-bottom:3px;">Revenue Opportunity</div>'
                f'<span class="rev-badge {rev_cls}">{rev_opp}</span>'
                f'</div>'
                f'<div style="font-size:11px;color:#64748b;margin-bottom:8px;">Growth Driver: <b>{driver}</b><br/>Campaign: <b>{campaign}</b></div>'
                f'<div style="margin-bottom:8px;">{pos_pills}{neg_pills}</div>'
                f'<div>{tag_html}</div>'
                f'</div>'
            )
        ...
        with r3:
            ...
            ap_rows = "".join([
                ...
                for k,v in [
                    ...
                    ("👥 Target Segment", ap['segment']),
                    ("📣 Main Channel", ap['channel']),
                    ("💬 Message", ap['message']),
                    ("📊 KPI to Track", ap['kpi']),
                    ("⚠️ Risk to Monitor", ap['risk']),
                ]
            ])
            st.markdown(
                f'<div style="background:#f8fafc;border-radius:12px;padding:14px 16px;">{ap_rows}</div>',
                unsafe_allow_html=True
            )
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown("""<div style="background:#fff;border-radius:16px;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.08);">
      <div style="font-size:13px;color:#475569;line-height:2.2;">
        ...
        <b style="color:#4f46e5;">Step 6 — Scenario Calibration (What-If only):</b> For the interactive What-If Scenario only, the app applies a bounded post-processing calibration layer so that user-controlled changes respond in economically intuitive directions. The calibration coefficients are intentionally capped and used only to improve the usability of user-controlled simulations. They do not replace the trained model and do not determine the main market ranking. This calibration is not used for model training, test evaluation, feature importance, confusion matrix, or the main dashboard ranking.
      </div>
    </div>""",unsafe_allow_html=True)
    st.markdown("---")

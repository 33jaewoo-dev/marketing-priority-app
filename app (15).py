...
# ── 상수 ──────────────────────────────────────────────────────────
FLAGS = {
    '중국': '<img src="https://flagcdn.com/w40/cn.png" width="36" style="border-radius:4px;">',
    '일본': '<img src="https://flagcdn.com/w40/jp.png" width="36" style="border-radius:4px;">',
    '대만': '<img src="https://flagcdn.com/w40/tw.png" width="36" style="border-radius:4px;">',
    '미국': '<img src="https://flagcdn.com/w40/us.png" width="36" style="border-radius:4px;">',
    '홍콩': '<img src="https://flagcdn.com/w40/hk.png" width="36" style="border-radius:4px;">',
}
FLAGS_SM = {
    '중국': '<img src="https://flagcdn.com/w20/cn.png" width="20" style="border-radius:2px;vertical-align:middle;">',
    '일본': '<img src="https://flagcdn.com/w20/jp.png" width="20" style="border-radius:2px;vertical-align:middle;">',
    '대만': '<img src="https://flagcdn.com/w20/tw.png" width="20" style="border-radius:2px;vertical-align:middle;">',
    '미국': '<img src="https://flagcdn.com/w20/us.png" width="20" style="border-radius:2px;vertical-align:middle;">',
    '홍콩': '<img src="https://flagcdn.com/w20/hk.png" width="20" style="border-radius:2px;vertical-align:middle;">',
}
...
def calc_priority_score(inp, df_ref):
    """입력값으로 Priority Score(0~100) 실시간 계산"""
    import numpy as np
    ...
    # 전체 데이터 범위를 기준으로 정규화
    v_score   = minmax(inp['visitor_count'],      df_ref['visitor_count'].min(),      df_ref['visitor_count'].max())
    mom       = max(-1, min(2, inp['visitor_mom_growth']))
    vs_avg    = max(-1, min(2, inp['visitor_vs_3m_avg'] - 1))
    g_raw     = mom * 0.6 + vs_avg * 0.4
    g_score   = minmax(g_raw, -1.0, 2.0)

    eng_n  = minmax(inp['engagement'],         df_ref['engagement'].min(),         df_ref['engagement'].max())
    exp_n  = minmax(inp['potential_exposure'],  df_ref['potential_exposure'].min(), df_ref['potential_exposure'].max())
    buz_n  = minmax(inp['buzz_volume'],         df_ref['buzz_volume'].min(),        df_ref['buzz_volume'].max())
    sns_score = eng_n * 0.5 + exp_n * 0.3 + buz_n * 0.2

    emom = max(-1, min(2, inp['engagement_mom_growth']))
    bmom = max(-1, min(2, inp['buzz_mom_growth']))
    sg_raw = emom * 0.5 + bmom * 0.5
    sg_score = minmax(sg_raw, -1.0, 2.0)

    h_score = minmax(inp['hallyu_spend_count'], df_ref['hallyu_spend_count'].min(), df_ref['hallyu_spend_count'].max())

    base = (0.30 * v_score + 0.25 * g_score + 0.20 * sns_score +
            0.15 * sg_score + 0.10 * h_score)

    # 감성 페널티
    pos = inp['positive_pct']
    if   pos < 40:  s_mult = 0.50
    elif pos < 55:  s_mult = 0.75
    elif pos < 65:  s_mult = 0.90
    else:           s_mult = 1.00

    # 급락 페널티
    vmom2 = inp['visitor_mom_growth']
    if   vmom2 < -0.50: sh_mult = 0.40
    elif vmom2 < -0.30: sh_mult = 0.60
    elif vmom2 < -0.15: sh_mult = 0.80
    else:               sh_mult = 1.00

    # 성수기·환율 보너스
    peak_b = inp['is_peak_season'] * 0.05
    fx_n   = minmax(inp['exchange_rate'], df_ref['exchange_rate'].min(), df_ref['exchange_rate'].max())
    fx_b   = fx_n * 0.03

    score = (base + peak_b + fx_b) * s_mult * sh_mult
    return round(min(max(score, 0), 1) * 100, 1)   # 0~100점
...
with st.sidebar:
    ...
    <div style="font-size:12px;color:#64748b;line-height:2;">
      ...
      <img src="https://flagcdn.com/w20/cn.png" width="18" style="border-radius:2px;margin:2px;">
      <img src="https://flagcdn.com/w20/jp.png" width="18" style="border-radius:2px;margin:2px;">
      <img src="https://flagcdn.com/w20/tw.png" width="18" style="border-radius:2px;margin:2px;">
      <img src="https://flagcdn.com/w20/us.png" width="18" style="border-radius:2px;margin:2px;">
      <img src="https://flagcdn.com/w20/hk.png" width="18" style="border-radius:2px;margin:2px;">
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    country_sel = st.selectbox("분석 국가 선택",['중국','일본','대만','미국','홍콩'],
                                format_func=lambda x:f"{CNAME[x]} ({x})")
    ...
    if st.button("🔮  우선순위 분석 실행", use_container_width=True):
        ...
        with r1:
            ...
            <div class="result-box" style="border-top:4px solid {clr};">
              <div style="margin-bottom:12px;">{flag}</div>
              ...
              <div style="font-size:12px;color:#94a3b8;margin-bottom:4px;">Priority Confidence</div>
              <div style="font-size:11px;color:#b0bec5;margin-bottom:16px;line-height:1.4;">
                이 국가를 <b>{lbl}</b> 우선순위로<br/>분류할 AI 확신도
              </div>
              <div>{tag_html}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    for i, ctry in enumerate(['중국','일본','대만','미국','홍콩']):
        ...
        with cols[i]:
            ...
            <div class="card" style="padding:20px 16px;border-top:4px solid {COLORS[i]};text-align:center;">
              <div style="margin-bottom:8px;">{FLAGS[ctry]}</div>

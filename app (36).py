...
st.set_page_config(page_title="Inbound Marketing Intelligence", page_icon="🎯",
...
.country-card { background:#fff; border-radius:16px; padding:20px 16px; text-align:center;
...
.up{color:#16a34a;font-weight:700;} .down{color:#dc2626;font-weight:700;} .neu{color:#d97706;font-weight:700;}
...
.comp-bar{height:8px;border-radius:4px;background:#e2e8f0;overflow:hidden;margin-top:4px;}
...
FLAG_URL = {'중국':'https://flagcdn.com/w40/cn.png','일본':'https://flagcdn.com/w40/jp.png',
            '대만':'https://flagcdn.com/w40/tw.png','미국':'https://flagcdn.com/w40/us.png',
...
INDUSTRY_EN = {
    ...
    'K-한식':'K-Food','한식':'K-Food','숙식':'Accommodation & Dining',
...
def inp_to_full_features(inp):
    return {f: float(inp.get(f, 0) or 0) for f in FEATS}

# ── Market Attractiveness Score (ML-informed composite) ───────────
def calc_attractiveness(inp, ml_high_prob, all_vc=None, all_mv=None, all_kw=None):
    """
    Market Attractiveness Score (0-100)
    ML-informed composite score combining:
      45% — ML High Priority Probability (Random Forest output)
      20% — Demand Score (visitor volume + momentum)
      15% — Digital Momentum Score (SNS engagement + buzz)
      10% — Sentiment Score (positive sentiment %)
      10% — Revenue / Korean Wave Score (proxy)
    """
    def mm(val, mn, mx):
        if mx <= mn: return 50.0
        return float(np.clip((val - mn) / (mx - mn) * 100, 0, 100))
    def pct_rank(val, vals):
        if not vals or len(vals) < 2: return 50.0
        return sum(1 for v in vals if v <= val) / len(vals) * 100

    ml_comp = float(np.clip(ml_high_prob * 100, 0, 100))

    vc   = inp.get('visitor_count', 0) or 0
    mom  = inp.get('visitor_mom_growth', 0) or 0
    vvs3 = inp.get('visitor_vs_3m_avg', 1) or 1
    vc_s   = pct_rank(vc, all_vc) if all_vc else mm(vc, 0, 600000)
    mom_s  = mm(mom, -0.5, 0.5)
    vs3_s  = mm(vvs3 - 1, -0.3, 0.5)
    demand_comp = vc_s*0.5 + mom_s*0.3 + vs3_s*0.2

    eng  = inp.get('engagement', 0) or 0
    bz   = inp.get('buzz_volume', 0) or 0
    emom = inp.get('engagement_mom_growth', 0) or 0
    bmom = inp.get('buzz_mom_growth', 0) or 0
    digital_comp = (mm(eng,0,4e6)*0.4 + mm(bz,0,500000)*0.2 +
                    mm(emom,-0.5,0.5)*0.2 + mm(bmom,-0.5,0.5)*0.2)

    sent_comp = float(np.clip(inp.get('positive_pct', 50) or 50, 0, 100))

    kw  = inp.get('hallyu_spend_count', 0) or 0
    sp  = inp.get('spend_per_person', 1000) or 1000
    mv  = vc * sp
    kw_s = pct_rank(kw, all_kw) if all_kw else mm(kw, 0, 2e6)
    mv_s = pct_rank(mv, all_mv) if all_mv else mm(mv, 0, 2e9)
    revenue_comp = kw_s*0.5 + mv_s*0.5

    score = (0.45*ml_comp + 0.20*demand_comp + 0.15*digital_comp +
             0.10*sent_comp + 0.10*revenue_comp)
    breakdown = {
        'ML Component (45%)':         round(ml_comp, 1),
        'Demand Score (20%)':         round(demand_comp, 1),
        'Digital Momentum (15%)':     round(digital_comp, 1),
        'Sentiment Score (10%)':      round(sent_comp, 1),
        'Revenue / K-Wave (10%)':     round(revenue_comp, 1),
    }
    return round(float(np.clip(score, 0, 100)), 1), breakdown
...
def calc_whatif_adjustment(v_chg, s_chg, bz_chg, se_chg, oi_chg, ex_chg):
    ...
    Smooth exponential curve — fully continuous.
    Not used for model training, evaluation, or main dashboard.
    """
    def cont(chg, cap, k):
        if chg == 0: return 0.0
        return round((1 if chg>0 else -1) * cap * (1 - np.exp(-k * abs(chg))), 2)
    # Since Market Attractiveness Score already reflects component changes directly
    # (e.g. sentiment change → Sentiment Score rises), the directional adjustment
    # is kept small — used only as a usability guard, not a primary scoring mechanism.
    v_adj  = cont(v_chg,  8,  0.05)   # cap reduced from 18
    s_adj  = cont(se_chg, 5,  0.07)   # cap reduced from 15 (sentiment already in component)
    e_adj  = cont(s_chg,  5,  0.04)   # cap reduced from 12
    bz_adj = cont(bz_chg, 4,  0.04)   # cap reduced from 10
    oi_adj = cont(-oi_chg,5,  0.04)   # cap reduced from 12
    ex_adj = cont(ex_chg, 5,  0.05)   # cap reduced from 12
    raw_total = v_adj+s_adj+e_adj+bz_adj+oi_adj+ex_adj
    # Apply 0.4 scaling so adjustment stays small relative to component-driven changes
    total  = round(float(np.clip(raw_total * 0.4, -10, 10)), 2)
    return total, {'visitor':v_adj,'sentiment':s_adj,'engagement':e_adj,
                   'buzz':bz_adj,'oil':oi_adj,'exchange':ex_adj}

# ── Revenue Opportunity ───────────────────────────────────────────
def _pct_rank(val, vals):
    if not vals or len(vals)<2: return 50.0
    return sum(1 for v in vals if v<=val)/len(vals)*100

def get_revenue_opportunity(ml_score, market_val, mom, engagement, kwave, sentiment,
                             all_market_vals=None, all_kwave_vals=None):
    mv_pct = _pct_rank(market_val, all_market_vals)/100.0 if all_market_vals else float(np.clip(market_val/2e9,0,1))
    kw_pct = _pct_rank(kwave, all_kwave_vals)/100.0 if all_kwave_vals else float(np.clip(kwave/1e6,0,1))
    score  = (ml_score*0.30 + mv_pct*100*0.25 + kw_pct*100*0.15 +
              float(np.clip((mom+0.5)/1.0,0,1))*100*0.15 +
              float(np.clip(engagement/4e6,0,1))*100*0.10 +
              float(np.clip(sentiment/100,0,1))*100*0.05)
...
def get_growth_driver_campaign(row_data, spend_per_person=0, revisit_rate=0,
    ...
    med_vc = float(np.median(all_vc)) if all_vc else 150000
    med_kw = float(np.median(all_kw)) if all_kw else 400000
    high_vc = cs>0.28 or vc>med_vc*1.3
    if mom<-0.15 and vc>30000: return "Recovery Opportunity","Recovery Campaign"
    if high_vc and mom>=-0.10:
        if kw>med_kw*1.5: return "Korean Wave-Driven Growth","K-content Campaign"
        return "Volume-Driven Growth","Acquisition Focus"
    if kw>med_kw*1.2: return "Korean Wave-Driven Growth","K-content Campaign"
    if (eng>300000 or bz>30000) and mom>=0: return "SNS Momentum-Driven Growth","Awareness-to-Conversion"
    if revisit_rate>68: return "Revisit-Driven Growth","Retention & Loyalty Campaign"
    if spend_per_person>1500 and not high_vc: return "Premium Spend-Driven Growth","Premium Upsell"
    return "Mixed-Signal Market","Balanced Campaign"

# ── Action Plans ──────────────────────────────────────────────────
ACTION_PLANS = {
    'Volume-Driven Growth': {
        'objective': 'Scale visitor acquisition to maximize absolute inbound traffic',
        'segment':   'Mass-market leisure travelers in the origin country',
        'channel':   'OTA partnerships, travel agencies, digital display & SNS ads',
        'message':   'Korea for everyone — variety, value, and culture',
        'kpi':       'Visitor MoM growth, market share, package booking volume',
        'risk':      'Geopolitical tensions, flight capacity constraints, visa policy changes',
    },
    'Korean Wave-Driven Growth': {
        'objective': 'Convert Korean Wave fans into inbound tourists',
        'segment':   'K-pop, K-drama, and K-beauty consumers aged 18–35',
        'channel':   'Social media influencers, K-content platforms, fan events',
        'message':   'Experience Korea beyond the screen — live the culture',
        'kpi':       'Korean Wave transaction growth, SNS engagement rate, influencer reach',
        'risk':      'K-content trend volatility, over-reliance on celebrity-driven demand',
    },
    'SNS Momentum-Driven Growth': {
        'objective': 'Capture rising digital interest and convert to bookings',
        'segment':   'Digitally active travelers responsive to short-form SNS content',
        'channel':   'TikTok, Instagram Reels, YouTube Shorts, UGC campaigns',
        'message':   'Korea is trending — be part of the moment',
        'kpi':       'Buzz volume MoM growth, engagement per visitor, click-through rate',
        'risk':      'SNS trend reversals, negative viral content, platform algorithm changes',
    },
    'Premium Spend-Driven Growth': {
        'objective': 'Increase revenue per visitor through premium segment targeting',
        'segment':   'High-income long-haul travelers, luxury and wellness seekers',
        'channel':   'Premium travel media, luxury agency partnerships, concierge services',
        'message':   'Korea redefined — luxury experiences beyond expectations',
        'kpi':       'Spend per person, stay duration, premium package conversion rate',
        'risk':      'Exchange rate strengthening, high airfare, premium product availability',
    },
    'Revisit-Driven Growth': {
        'objective': 'Strengthen loyalty and increase repeat visit frequency',
        'segment':   'Previous Korea visitors with high satisfaction and revisit intent',
        'channel':   'CRM / loyalty programs, targeted email, returning visitor offers',
        'message':   'There is always more to discover — come back to Korea',
        'kpi':       'Revisit rate, recommend intention, loyalty program enrollment',
        'risk':      'Destination fatigue, regional competition from Japan and Thailand',
    },
    'Recovery Opportunity': {
        'objective': 'Rebuild visitor volume after demand disruption',
        'segment':   'Lapsed visitors and new travelers in recovering markets',
        'channel':   'PR campaigns, travel incentive programs, direct airline partnerships',
        'message':   'Korea is welcoming you back — better than ever',
        'kpi':       'Visitor recovery rate vs. pre-disruption baseline, MoM rebound speed',
        'risk':      'Sustained diplomatic tensions, slow aviation recovery, perception barriers',
    },
    'Mixed-Signal Market': {
        'objective': 'Maintain presence while monitoring for clearer priority signals',
        'segment':   'Broad audience — no single dominant segment identified',
        'channel':   'Balanced mix of digital and traditional channels',
        'message':   'Korea has something for everyone',
        'kpi':       'Overall visitor growth, SNS engagement trend, sentiment stability',
        'risk':      'Unclear campaign return, budget dilution across segments',
    },
}
...
def load_model():
    ...
    X_tr=dc[train_mask][FEATS]; X_te=dc[test_mask][FEATS]
    y_tr=le.fit_transform(dc[train_mask]['priority_label'])
    y_te=le.transform(dc[test_mask]['priority_label'])
    rf = RandomForestClassifier(n_estimators=200,max_depth=10,random_state=42,class_weight='balanced')
    rf.fit(X_tr, y_tr)
    rf_pred = rf.predict(X_te)
    rf_acc  = accuracy_score(y_te, rf_pred)
    cm      = confusion_matrix(y_te, rf_pred)
    cr      = classification_report(y_te,rf_pred,target_names=le.classes_,
                                    output_dict=True,labels=list(range(len(le.classes_))),zero_division=0)
    sc=StandardScaler(); lr=LogisticRegression(max_iter=1000,class_weight='balanced',random_state=42)
    ...
    rule_acc=accuracy_score(y_te,dc[test_mask].apply(rule_fn,axis=1))
    fi=pd.DataFrame({'feature':FEATS,'importance':rf.feature_importances_}).sort_values('importance',ascending=False)
    met={'rf_acc':rf_acc,'lr_acc':lr_acc,'rule_acc':rule_acc,'cm':cm,'cr':cr,
         'classes':le.classes_,'train_n':len(X_tr),'test_n':len(X_te)}
    return rf, le, fi, met
...
with st.sidebar:
    ...
    page = st.radio("",["📊  Overview","🔮  Priority Engine","💰  Budget Planner",
                         "🔀  Compare Markets","👤  Market Profiles","📈  Analytics","🔬  Methodology"],
...
# ── Precompute latest data ─────────────────────────────────────────
latest_raw = df[df['year_month']==latest_month].copy()
_sat_last  = sat_df.sort_values('year').groupby('country').last().reset_index()
_spend_map = {r['country']: float(r['spend_per_person_usd']) for _,r in _sat_last.iterrows()}
_all_mv    = [float(latest_raw[latest_raw['country']==c]['visitor_count'].values[0] if len(latest_raw[latest_raw['country']==c])>0 else 0)*_spend_map.get(c,0) for c in CNAME]
_all_kw    = [float(latest_raw[latest_raw['country']==c]['hallyu_spend_count'].values[0] if len(latest_raw[latest_raw['country']==c])>0 else 0) for c in CNAME]
_all_vc    = [float(latest_raw[latest_raw['country']==c]['visitor_count'].values[0] if len(latest_raw[latest_raw['country']==c])>0 else 0) for c in CNAME]

latest_computed = []
for _, row in latest_raw.iterrows():
    inp  = row_to_full_features(row)
    pred, pd_, pred_prob = get_ml_prediction(inp)
    hl   = confidence_label(pred_prob)
    ...
    inp['spend_per_person'] = spend
    mv   = float(row.get('visitor_count',0) or 0)*spend
    kwave= float(row.get('hallyu_spend_count',0) or 0)
    ml_high_prob = pd_['High']
    attr_score, attr_bd = calc_attractiveness(inp, ml_high_prob, _all_vc, _all_mv, _all_kw)
    rev_opp, rev_cls = get_revenue_opportunity(attr_score, mv,
        float(row.get('visitor_mom_growth',0) or 0),
        float(row.get('engagement',0) or 0), kwave,
        float(row.get('positive_pct',50) or 50),
        all_market_vals=_all_mv, all_kwave_vals=_all_kw)
    driver, campaign = get_growth_driver_campaign(row, spend, revisit, _all_mv, _all_vc, _all_kw)
    pos_d, neg_d = get_market_signals(inp)
    latest_computed.append({
        'country':row['country'],'pred':pred,'prob_dict':pd_,'pred_prob':pred_prob,
        'conf_label':hl[0],'conf_cls':hl[1],
        'attr_score':attr_score,'attr_bd':attr_bd,
        'ml_high_prob':ml_high_prob,
...
latest_computed.sort(key=lambda x: -x['attr_score'])
...
if page == "📊  Overview":
    ...
    <p style="color:#64748b;font-size:14px;margin-bottom:8px;">The core business question is which inbound market can serve as the strongest growth driver under limited marketing resources. This system supports data-driven market prioritization combining visitor demand, SNS attention, Korean Wave consumption, and macro indicators.</p>""",unsafe_allow_html=True)
    st.markdown("""<div class="insight-box"><b>How to use:</b> (1) Check Market Attractiveness Ranking → (2) Review visitor &amp; SNS trends → (3) Open <b>Priority Engine</b> for scenario simulation → (4) Use <b>Budget Planner</b> for allocation → (5) Use <b>Compare Markets</b> for head-to-head analysis</div>""",unsafe_allow_html=True)

    st.markdown(f'<div class="lbl" style="margin-top:16px;">Market Attractiveness Ranking — {ym_disp} · ML-Informed Composite Score</div>',unsafe_allow_html=True)
    cols = st.columns(5)
    for i, r in enumerate(latest_computed):
        lbl=r['pred']; clr=PCOLOR[lbl]; rank=['1st','2nd','3rd','4th','5th'][i]
        mom=r['visitor_mom_growth']; ms=r['attr_score']
        ...
        with cols[i]:
            st.markdown(
                f'<div class="country-card {PBDR[lbl]}">'
                f'<div style="font-size:10px;color:#94a3b8;letter-spacing:.1em;text-transform:uppercase;margin-bottom:8px;">{rank}</div>'
                f'<div style="margin-bottom:10px;">{flag(r["country"],36)}</div>'
                f'<div style="font-family:Outfit,sans-serif;font-size:15px;font-weight:700;color:#0f172a;margin-bottom:8px;">{CNAME[r["country"]]}</div>'
                f'<span class="badge {PBDR[lbl]}">{lbl}</span>'
                f'<div style="margin-top:12px;padding-top:10px;border-top:1px solid #f1f5f9;">'
                f'<div style="font-size:10px;color:#94a3b8;margin-bottom:2px;">Market Attractiveness Score</div>'
                f'<div style="font-family:Outfit,sans-serif;font-size:26px;font-weight:800;color:{clr};">{ms:.1f}<span style="font-size:13px;color:#94a3b8;">/100</span></div>'
                f'<div style="font-size:11px;color:#94a3b8;">ML High Prob: <b style="color:{clr};">{r["ml_high_prob"]*100:.0f}%</b></div>'
                f'<div style="margin:4px 0;"><span class="{r["conf_cls"]}">{r["conf_label"]} Confidence</span></div>'
                f'<div style="font-size:11px;font-weight:600;color:{"#16a34a" if mom>0 else "#dc2626"};margin-top:4px;">{"▲" if mom>0 else "▼"} {abs(mom*100):.1f}% MoM</div>'
                f'<div style="font-size:11px;color:#94a3b8;">{int(r["visitor_count"]):,} visitors</div>'
                f'</div>'
                f'<div style="margin-top:8px;padding-top:8px;border-top:1px solid #f1f5f9;text-align:left;">'
                f'<div style="font-size:10px;color:#94a3b8;margin-bottom:3px;">Revenue Opportunity Proxy</div>'
                f'<span class="rev-badge {r["rev_cls"]}">{r["rev_opp"]}</span>'
                f'<div style="font-size:10px;color:#94a3b8;margin-top:2px;">Market Value Proxy: {mv_disp}</div>'
                f'<div style="font-size:10px;color:#64748b;margin-top:2px;">{r["driver"][:26]}</div>'
                f'</div>'
                f'<div style="margin-top:8px;padding-top:8px;border-top:1px solid #f1f5f9;text-align:left;">'
                f'<div style="font-size:10px;color:#94a3b8;margin-bottom:4px;">Observable Market Signals</div>'
                f'{dh if dh else chr(10).join(["<span style=",chr(39),"font-size:11px;color:#94a3b8;",chr(39),">No dominant signal</span>"])}'
                f'<div style="font-size:9px;color:#cbd5e1;margin-top:4px;font-style:italic;">Not causal attribution</div>'
                f'</div></div>',
                unsafe_allow_html=True)

    st.markdown("""<div class="proxy-note" style="margin-top:8px;">
      <b>Note:</b> Market Attractiveness Score is an ML-informed composite score combining Random Forest High Priority probability (45%), visitor demand (20%), digital momentum (15%), sentiment (10%), and revenue/Korean Wave proxy (10%). Revenue Opportunity is a directional proxy — not actual campaign revenue or guaranteed ROI.
    </div>""",unsafe_allow_html=True)
    ...
    for col,l,v,s in [(mc[0],"Algorithm","Random Forest","200 estimators · depth 10"),
        ...
        with col:
            st.markdown(f'<div class="card" style="padding:18px 20px;"><div style="font-size:10px;color:#94a3b8;letter-spacing:.1em;text-transform:uppercase;margin-bottom:8px;">{l}</div><div style="font-family:Outfit,sans-serif;font-size:19px;font-weight:700;color:#0f172a;">{v}</div><div style="font-size:11px;color:#94a3b8;margin-top:4px;">{s}</div></div>',unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    <p style="color:#64748b;font-size:14px;margin-bottom:16px;">Adjust market indicators — the Random Forest model predicts priority class, and the Market Attractiveness Score combines ML output with observable business signals.</p>""",unsafe_allow_html=True)
    mode = st.radio("",["📋  Standard Input","⚡  What-If Scenario"],horizontal=True,label_visibility="collapsed")
    st.markdown("<br/>",unsafe_allow_html=True)
    country_sel = st.selectbox("Select Market",list(CNAME.keys()),format_func=lambda x:CNAME[x])
    ...
    if "Standard" in mode:
        ...
        with t2:
            c1,c2=st.columns(2)
            with c1: buzz_volume=st.number_input("Buzz Volume",value=int(cd['buzz_volume']),step=100); engagement=st.number_input("Engagement",value=int(cd['engagement']),step=1000)
            with c2:
                ...
                positive_pct=st.slider("Positive Sentiment (%)",0.0,100.0,float(cd['positive_pct']),0.5,
                    help="Sentiment input signal — directly contributes to the Sentiment component of Market Attractiveness Score.")
        ...
        bl1=float(cd.get('buzz_lag1',buzz_volume) or buzz_volume); el1=float(cd.get('engagement_lag1',engagement) or engagement)
        exl1=float(cd.get('exposure_lag1',potential_exposure) or potential_exposure)
        bmom=(buzz_volume-bl1)/bl1 if bl1 else 0; emom=(engagement-el1)/el1 if el1 else 0
        ...
        st.markdown("""<div class="proxy-note">
          The Market Attractiveness Score will change as you adjust the sliders.<br/>
          <b>Raw ML High Priority Probability</b> comes from Random Forest (29 features).<br/>
          A <b>bounded directional calibration</b> is applied on top for scenario usability only —
          not used for model training, evaluation, feature importance, or main dashboard ranking.
        </div>""",unsafe_allow_html=True)
        cd_full = row_to_full_features(cd)
        cd_full['spend_per_person'] = spend
        c1,c2=st.columns(2)
        with c1:
            v_chg =st.slider("Visitor Count Change (%)",-50,50,0,1)
            s_chg =st.slider("SNS Engagement Change (%)",-50,50,0,1)
            bz_chg=st.slider("Buzz Volume Change (%)",-50,50,0,1)
        with c2:
            se_chg=st.slider("Positive Sentiment Change (pp)",-20,20,0,1)
            oi_chg=st.slider("Oil Price Change (%)",-30,50,0,1)
            ex_chg=st.slider("Exchange Rate Change (%)",-20,20,0,1)
        inp = cd_full.copy()
        ...
        el1b=float(cd_full.get('engagement_lag1',b_eng) or b_eng)
        bl1b=float(cd_full.get('buzz_lag1',b_bz) or b_bz)
        b3mb=float(cd_full.get('buzz_3m_avg',b_bz) or b_bz)
        inp['engagement_mom_growth']  = (inp['engagement']-el1b)/el1b if el1b else 0
        inp['engagement_per_visitor'] = inp['engagement']/inp['visitor_count'] if inp['visitor_count'] else 0
        inp['buzz_mom_growth']   = (inp['buzz_volume']-bl1b)/bl1b if bl1b else 0
        inp['buzz_vs_3m_avg']    = inp['buzz_volume']/b3mb if b3mb else 1
        inp['buzz_per_visitor']  = inp['buzz_volume']/inp['visitor_count'] if inp['visitor_count'] else 0
        hc_b=float(cd_full.get('hallyu_spend_count',0) or 0)
        inp['hallyu_per_visitor']= hc_b/inp['visitor_count'] if inp['visitor_count'] else 0
        is_whatif = True

    st.markdown("<br/>",unsafe_allow_html=True)
    if st.button("🤖  Run Market Priority Analysis", use_container_width=True):
        pred, pd_, pred_prob = get_ml_prediction(inp)
        ...
        # Baseline
        cd_full2 = row_to_full_features(cd); cd_full2['spend_per_person']=spend
        base_pred, base_pd_, _ = get_ml_prediction(cd_full2)
        base_attr, _ = calc_attractiveness(cd_full2, base_pd_['High'], _all_vc, _all_mv, _all_kw)

        # Raw attractiveness score
        raw_attr, attr_bd = calc_attractiveness(inp, ml_high_prob, _all_vc, _all_mv, _all_kw)

        # What-if adjustment
        adj_total=0; adj_bd={}
        if is_whatif:
            adj_total, adj_bd = calc_whatif_adjustment(v_chg,s_chg,bz_chg,se_chg,oi_chg,ex_chg)

        # All-zero guard: if no slider moved, show exactly baseline
        active_sliders = sum(1 for x in [v_chg,s_chg,bz_chg,se_chg,oi_chg,ex_chg] if x!=0)
        if is_whatif and active_sliders == 0:
            raw_attr = base_attr
            adj_total = 0.0

        # Sanity correction (single variable)
        final_attr = float(np.clip(raw_attr + adj_total, 0, 100))
        if is_whatif:
            active = sum(1 for x in [v_chg,s_chg,bz_chg,se_chg,oi_chg,ex_chg] if x!=0)
            if active == 1:
                def mv_guard(chg, adj_val, cap=2.0):
                    return min(cap, abs(adj_val)*0.5) if chg!=0 else 0
                if v_chg>0:   final_attr=max(final_attr, base_attr+mv_guard(v_chg,adj_bd.get('visitor',0),2.0))
                if se_chg>0:  final_attr=max(final_attr, base_attr+mv_guard(se_chg,adj_bd.get('sentiment',0),2.0))
                if s_chg>0:   final_attr=max(final_attr, base_attr+mv_guard(s_chg,adj_bd.get('engagement',0),1.5))
                if bz_chg>0:  final_attr=max(final_attr, base_attr+mv_guard(bz_chg,adj_bd.get('buzz',0),1.0))
                if ex_chg>0:  final_attr=max(final_attr, base_attr+mv_guard(ex_chg,adj_bd.get('exchange',0),1.5))
                if v_chg<0:   final_attr=min(final_attr, base_attr-mv_guard(v_chg,adj_bd.get('visitor',0),2.0))
                if se_chg<0:  final_attr=min(final_attr, base_attr-mv_guard(se_chg,adj_bd.get('sentiment',0),2.0))
                if s_chg<0:   final_attr=min(final_attr, base_attr-mv_guard(s_chg,adj_bd.get('engagement',0),1.5))
                if oi_chg>0:  final_attr=min(final_attr, base_attr-mv_guard(oi_chg,abs(adj_bd.get('oil',0)),1.5))
                if oi_chg<0:  final_attr=max(final_attr, base_attr+mv_guard(oi_chg,abs(adj_bd.get('oil',0)),1.5))
                if ex_chg<0:  final_attr=min(final_attr, base_attr-mv_guard(ex_chg,adj_bd.get('exchange',0),1.5))
            final_attr = round(float(np.clip(final_attr,0,100)),1)

        diff = round(final_attr - base_attr, 1)
        score_display = final_attr if is_whatif else raw_attr

        mv  = inp['visitor_count']*spend
        kwave=inp.get('hallyu_spend_count',0) or 0
        rev_opp,rev_cls=get_revenue_opportunity(score_display,mv,inp.get('visitor_mom_growth',0),
            inp.get('engagement',0),kwave,inp.get('positive_pct',50),_all_mv,_all_kw)
        try:
            _pe_vc=[float(latest_raw[latest_raw['country']==c]['visitor_count'].values[0]) if len(latest_raw[latest_raw['country']==c])>0 else 0 for c in CNAME]
            driver,campaign=get_growth_driver_campaign(inp,spend,revisit,_all_mv,_pe_vc,_all_kw)
        except:
        ...
        with r1:
            ...
            score_label="Scenario-Adjusted Market Attractiveness Score" if is_whatif else "Market Attractiveness Score"
            sub_label=("Component-driven score with small scenario usability calibration" if is_whatif
                       else "ML (45%) + Demand (20%) + Digital (15%) + Sentiment (10%) + K-Wave (10%)")

            if is_whatif:
                scenario_tier=("High Scenario Signal" if score_display>=65 else
                               "Medium Scenario Signal" if score_display>=40 else "Low Scenario Signal")
                adj_info=(f'<div style="font-size:11px;color:#94a3b8;margin-top:3px;">'
                          f'Base Score: {raw_attr:.1f} · Scenario calibration: {adj_total:+.1f} pts</div>'
                          f'<div style="font-size:9px;color:#94a3b8;font-style:italic;margin-top:2px;">'
                          f'Adjustment for scenario simulation only — not part of model evaluation</div>'
                          f'<div style="margin-top:4px;">'
                          f'<span style="background:#f1f5f9;border-radius:6px;padding:2px 8px;font-size:10px;color:#475569;">Scenario Tier: {scenario_tier}</span></div>')
            else:
                adj_info=""

            html_card = (
                f'<div class="result-box" style="border-top:4px solid {clr};">'
                f'<div style="margin-bottom:12px;">{flag(country_sel,48)}</div>'
                f'<div style="font-family:Outfit,sans-serif;font-size:22px;font-weight:700;color:#0f172a;margin-bottom:12px;">{cn}</div>'
                f'<span class="badge {PBDR[lbl]}" style="font-size:13px;padding:5px 16px;">{lbl} Priority</span>'
                f'&nbsp;<span class="{conf_cls}">{conf_l} Confidence</span>'
                f'<div style="margin:18px 0 6px;">'
                f'<div style="font-size:10px;color:#94a3b8;letter-spacing:.1em;text-transform:uppercase;margin-bottom:3px;">{score_label}</div>'
                f'<div style="font-size:10px;color:#94a3b8;margin-bottom:4px;">{sub_label}</div>'
                f'<div style="font-family:Outfit,sans-serif;font-size:58px;font-weight:800;color:{clr};letter-spacing:-.04em;line-height:1;">{score_display}</div>'
                f'<div style="font-size:14px;color:#94a3b8;margin-bottom:4px;">/ 100</div>'
                + adj_info +
                f'<div style="margin-top:6px;">{diff_html}</div>'
                f'</div>'
                f'<div style="background:#f1f5f9;border-radius:8px;height:10px;margin:10px 0 4px;overflow:hidden;">'
                f'<div style="width:{score_display}%;height:100%;background:linear-gradient(90deg,{clr}88,{clr});border-radius:8px;"></div>'
                f'</div>'
                f'<div style="display:flex;justify-content:space-between;font-size:10px;color:#94a3b8;margin-bottom:12px;"><span>0</span><span>50</span><span>100</span></div>'
                f'<div style="margin-bottom:6px;">'
                f'<div style="font-size:10px;color:#94a3b8;margin-bottom:3px;">Revenue Opportunity Proxy</div>'
                f'<span class="rev-badge {rev_cls}">{rev_opp}</span>'
                f'</div>'
                f'<div style="font-size:11px;color:#64748b;margin-bottom:6px;">Growth Driver: <b>{driver}</b></div>'
                f'<div style="margin-bottom:6px;">{pos_pills}{neg_pills}</div>'
                f'<div>{tag_html}</div>'
                f'</div>'
            )
            ...
            st.markdown('<div class="lbl" style="margin-top:12px;">Score Components</div>',unsafe_allow_html=True)
            comp_colors = {'ML Component (45%)':'#4f46e5','Demand Score (20%)':'#10b981',
                           'Digital Momentum (15%)':'#f59e0b','Sentiment Score (10%)':'#ef4444',
                           'Revenue / K-Wave (10%)':'#8b5cf6'}
            for k,v in attr_bd.items():
                clr_c = comp_colors.get(k,'#94a3b8')
                st.markdown(
                    f'<div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:5px;">'
                    f'<span style="color:#64748b;">{k}</span>'
                    f'<span style="font-weight:600;color:{clr_c};">{v:.1f}</span></div>'
                    f'<div class="comp-bar"><div class="comp-fill" style="width:{v}%;background:{clr_c};"></div></div>',
                    unsafe_allow_html=True)

            # What-if interpretation
            if is_whatif:
                interps=[]
                if v_chg!=0: interps.append("Higher visitor count strengthens demand signals." if v_chg>0 else "Lower visitor count weakens demand signals.")
                if se_chg!=0: interps.append("Improved sentiment lifts the sentiment component." if se_chg>0 else "Lower sentiment reduces the sentiment component.")
                if s_chg!=0: interps.append("Higher SNS engagement improves digital momentum." if s_chg>0 else "Lower engagement weakens digital momentum.")
                if oi_chg!=0: interps.append("Higher oil prices add travel cost pressure." if oi_chg>0 else "Lower oil prices ease travel cost pressure.")
                if ex_chg!=0: interps.append("A weaker KRW improves Korea's affordability." if ex_chg>0 else "A stronger KRW reduces Korea's affordability.")
                if interps:
                    adj_sign="increases" if adj_total>=0 else "decreases"
                    st.markdown(f'<div class="insight-box" style="margin-top:10px;font-size:12px;"><b>Scenario:</b> The Market Attractiveness Score {adj_sign} by {abs(diff):.1f} pts. {" ".join(interps[:2])}</div>',unsafe_allow_html=True)

        with r2:
            st.markdown('<div class="lbl" style="margin-top:0;">Key Market Signals</div>',unsafe_allow_html=True)
            st.markdown('<div style="font-size:12px;color:#94a3b8;margin-bottom:8px;">Observable signals used as ML inputs and score components. Not causal attribution.</div>',unsafe_allow_html=True)
            def scls(v,t=0.05):
                if v>t: return "up","▲"
                if v<-t: return "down","▼"
                return "neu","━"
            sigs=[("Visitor MoM Growth",inp.get('visitor_mom_growth',0),f"{inp.get('visitor_mom_growth',0)*100:+.1f}%","MoM change"),
                  ...
                  ("Positive Sentiment",(inp.get('positive_pct',50)-55)/100,f"{inp.get('positive_pct',50):.1f}%","Sentiment component input"),
                  ("Market Share",(inp.get('country_share',0.2)-0.2)/0.2,f"{inp.get('country_share',0.2)*100:.1f}%","Share of 5 markets")]
            html_sigs=""
            for nm,vl,dp,dc in sigs:
                cls,arrow=scls(vl)
                html_sigs+=f'<div class="sig-row"><div><div style="color:#475569;font-weight:500;">{nm}</div><div style="font-size:11px;color:#94a3b8;">{dc}</div></div><div class="{cls}">{arrow} {dp}</div></div>'
            st.markdown(html_sigs,unsafe_allow_html=True)
            st.markdown("<br/>",unsafe_allow_html=True)
            st.markdown('<div style="font-size:11px;color:#94a3b8;margin-bottom:8px;">ML Classification Probability (Random Forest)</div>',unsafe_allow_html=True)
            for cn_l,p in [('High',pd_['High']),('Medium',pd_['Medium']),('Low',pd_['Low'])]:
                c2c=PCOLOR[cn_l]
                st.markdown(f'<div style="display:flex;align-items:center;gap:8px;margin-bottom:7px;"><div style="width:56px;font-size:12px;color:#64748b;">{cn_l}</div><div style="flex:1;background:#f1f5f9;border-radius:4px;height:8px;overflow:hidden;"><div style="width:{int(p*100)}%;height:100%;background:{c2c};border-radius:4px;"></div></div><div style="width:36px;text-align:right;font-family:Outfit,sans-serif;font-size:12px;font-weight:700;color:{c2c};">{int(p*100)}%</div></div>',unsafe_allow_html=True)

        with r3:
            clr2=PCOLOR[lbl]; bc=PBDR[lbl]; cn3=CNAME[country_sel]
            RECS={'High':("Aggressive Offensive Strategy","All key signals are strong — prioritize this market opportunity.","Increase Budget","#eef2ff"),
                  'Medium':("Monitor & Optimize","Mid-tier signals. Watch for momentum before scaling.","Maintain Budget","#fffbeb"),
                  'Low':("Reallocate & Monitor","Weak signals. Redirect resources to stronger markets.","Minimize Budget","#f8fafc")}
            title,desc,blbl,bg=RECS[lbl]
            actions=[]
            if lbl=='High':
                actions+=[("Budget Expansion",f"Increase {cn3}-specific ad spend 20–30% vs. last month"),
                           ("Campaign Type",f"Recommended: {campaign}")]
                ...
                actions+=[("Reallocate",f"Shift {cn3} budget to higher-priority markets"),
                           ("Root Cause Analysis","Diagnose: geopolitical tensions, flight capacity, or seasonal?")]
            ...
            st.markdown(f'<div style="background:{bg};border:1px solid {clr2}33;border-radius:12px;padding:16px 18px;margin-bottom:14px;"><div style="font-family:Outfit,sans-serif;font-size:16px;font-weight:700;color:{clr2};margin-bottom:6px;">{title}</div><div style="font-size:12px;color:#64748b;line-height:1.5;margin-bottom:10px;">{desc}</div><span style="background:{clr2};color:#fff;border-radius:6px;padding:3px 10px;font-size:11px;font-weight:600;">{blbl}</span></div>',unsafe_allow_html=True)
            ...
            # Action Plan
            ap = ACTION_PLANS.get(driver, ACTION_PLANS['Mixed-Signal Market'])
            st.markdown('<div class="lbl" style="margin-top:12px;">Action Plan</div>',unsafe_allow_html=True)
            ap_rows="".join([
                f'<div style="display:flex;gap:8px;padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:12px;">'
                f'<span style="color:#94a3b8;font-weight:600;min-width:120px;">{k}</span>'
                f'<span style="color:#334155;">{v}</span></div>'
                for k,v in [("🎯 Objective",ap['objective']),("👥 Target Segment",ap['segment']),
                            ("📣 Main Channel",ap['channel']),("💬 Message",ap['message']),
                            ("📊 KPI to Track",ap['kpi']),("⚠️ Risk to Monitor",ap['risk'])]])
            st.markdown(f'<div style="background:#f8fafc;border-radius:12px;padding:14px 16px;">{ap_rows}</div>',unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    for r in latest_computed:
        alloc_data.append({'country':r['country'],'country_en':CNAME[r['country']],'pred':r['pred'],
            'attr_score':r['attr_score'],'ml_high_prob':r['ml_high_prob'],
            'conf_label':r['conf_label'],'conf_cls':r['conf_cls'],
            'rev_opp':r['rev_opp'],'rev_cls':r['rev_cls'],'driver':r['driver'],'campaign':r['campaign'],
            'market_value':r['market_value']})
    alloc_df=pd.DataFrame(alloc_data).sort_values('attr_score',ascending=False)
    alloc_df['floor']=alloc_df['pred'].map(floors)
    score_sum=alloc_df['attr_score'].sum(); remaining=1-alloc_df['floor'].sum()
    alloc_df['extra']=alloc_df['attr_score']/score_sum*remaining
    ...
    for i,(_,r) in enumerate(alloc_df.iterrows()):
        ...
        with cols[i]:
            st.markdown(
                f'<div class="country-card {PBDR[r["pred"]]}">'
                f'<div style="margin-bottom:8px;">{flag(r["country"],32)}</div>'
                f'<div style="font-family:Outfit,sans-serif;font-size:14px;font-weight:700;color:#0f172a;margin-bottom:6px;">{r["country_en"]}</div>'
                f'<span class="badge {PBDR[r["pred"]]}">{r["pred"]}</span>'
                f'<div style="margin-top:10px;">'
                f'<div style="font-size:10px;color:#94a3b8;">Recommended Share</div>'
                f'<div style="font-family:Outfit,sans-serif;font-size:22px;font-weight:800;color:{clr};">{r["share"]*100:.1f}%</div>'
                f'<div style="font-size:13px;font-weight:600;color:#334155;">{fmt}</div>'
                f'<div style="font-size:11px;color:#94a3b8;margin-top:3px;">Attractiveness: {r["attr_score"]:.1f} · ML High: {r["ml_high_prob"]*100:.0f}%</div>'
                f'<div style="margin:3px 0;"><span class="{r["conf_cls"]}">{r["conf_label"]} Confidence</span></div>'
                f'<span class="rev-badge {r["rev_cls"]}">{r["rev_opp"]}</span>'
                f'<div style="font-size:10px;color:#94a3b8;margin-top:2px;">Mkt Value: {mv_disp}</div>'
                f'<div style="font-size:10px;color:#64748b;margin-top:2px;">{r["driver"][:25]}</div>'
                f'</div></div>',
                unsafe_allow_html=True)

    st.markdown("""<div class="proxy-note" style="margin-top:8px;">Revenue Opportunity and Market Value Proxy are directional proxies, not actual campaign revenue or guaranteed ROI forecasts.</div>""",unsafe_allow_html=True)
    st.markdown("<br/>",unsafe_allow_html=True)
    c1,c2=st.columns(2)
    with c1:
    ...
    disp_df=alloc_df[['country_en','pred','attr_score','ml_high_prob','conf_label','rev_opp','driver','campaign','share','budget']].copy()
    disp_df.columns=['Market','Priority','Attractiveness Score','ML High Prob','Confidence','Rev Opportunity','Growth Driver','Campaign Type','Share (%)','Budget (KRW)']
    disp_df['Share (%)']=disp_df['Share (%)'].multiply(100).round(1)
    disp_df['ML High Prob']=disp_df['ML High Prob'].multiply(100).round(1).astype(str)+'%'
    disp_df['Budget (KRW)']=disp_df['Budget (KRW)'].apply(lambda x:f"₩{x:,.0f}")
    disp_df['Attractiveness Score']=disp_df['Attractiveness Score'].round(1)
    if currency!='KRW (₩)':
        currency_code='USD' if 'USD' in currency else 'JPY'
        disp_df[f'Budget ({currency_code})']=alloc_df['budget'].div(krw_per_unit).apply(lambda x:f"{sym}{x:,.0f}")
...
# ══════════════════════════════════════════════════════════════════
    ...
    c1,c2=st.columns(2)
    with c1: mkt_a=st.selectbox("Market A",list(CNAME.keys()),index=0,format_func=lambda x:CNAME[x])
    with c2: mkt_b=st.selectbox("Market B",list(CNAME.keys()),index=1,format_func=lambda x:CNAME[x])

    if mkt_a!=mkt_b:
        da=next((x for x in latest_computed if x['country']==mkt_a),None)
        db=next((x for x in latest_computed if x['country']==mkt_b),None)
        if da and db:
            c1,c2=st.columns(2)
            for col,d,ctry in [(c1,da,mkt_a),(c2,db,mkt_b)]:
                ...
                with col:
                    rows_html="".join([f'<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #f1f5f9;font-size:13px;"><span style="color:#64748b;">{k}</span><span style="font-weight:600;color:#0f172a;">{v}</span></div>' for k,v in [
                        ("Attractiveness Score",f"{d['attr_score']:.1f}/100"),
                        ("ML High Probability",f"{d['ml_high_prob']*100:.0f}%"),
                        ("Predicted Priority",d['pred']),
                        ("Confidence",d['conf_label']),
                        ("Revenue Opportunity",d['rev_opp']),
                        ("Market Value Proxy",mv_disp),
                        ("Current Visitors",f"{int(d['visitor_count']):,}"),
                        ("MoM Growth",f"{d['visitor_mom_growth']*100:+.1f}%"),
                        ("Spend per Person",f"${d['spend']:,.0f}"),
                        ("Stay Days",f"{d['stay']:.0f} days"),
                        ("SNS Engagement",f"{int(d['row_data'].get('engagement',0) or 0):,}"),
                        ("Korean Wave Spend",f"{int(d['row_data'].get('hallyu_spend_count',0) or 0):,}"),
                        ("Revisit Rate",f"{d['revisit']:.0f}%"),
                        ("Growth Driver",d['driver'][:28]),
                        ("Campaign Type",d['campaign'][:28]),
                    ]])
                    st.markdown(f'<div class="card" style="border-top:4px solid {clr};"><div style="text-align:center;margin-bottom:16px;">{flag(ctry,48)}</div><div style="font-family:Outfit,sans-serif;font-size:20px;font-weight:700;color:#0f172a;text-align:center;margin-bottom:12px;">{CNAME[ctry]}</div><div style="display:flex;justify-content:center;gap:8px;margin-bottom:16px;"><span class="badge {PBDR[d["pred"]]}">{d["pred"]}</span><span class="{d["conf_cls"]}">{d["conf_label"]} Confidence</span></div>{rows_html}</div>',unsafe_allow_html=True)

            cn_a=CNAME[mkt_a]; cn_b=CNAME[mkt_b]; interps=[]
            if da['attr_score']>db['attr_score']+5: interps.append(f"{cn_a} has a higher Market Attractiveness Score ({da['attr_score']:.1f} vs {db['attr_score']:.1f}), suggesting higher overall marketing priority.")
            elif db['attr_score']>da['attr_score']+5: interps.append(f"{cn_b} has a higher Market Attractiveness Score ({db['attr_score']:.1f} vs {da['attr_score']:.1f}), suggesting higher overall marketing priority.")
            else: interps.append(f"{cn_a} and {cn_b} have similar Market Attractiveness Scores ({da['attr_score']:.1f} vs {db['attr_score']:.1f}).")
            ...
            if da['visitor_count']>db['visitor_count']*1.5: interps.append(f"{cn_a} offers stronger volume-driven growth potential ({int(da['visitor_count']):,} vs {int(db['visitor_count']):,} visitors).")
            elif db['visitor_count']>da['visitor_count']*1.5: interps.append(f"{cn_b} offers stronger volume-driven growth potential ({int(db['visitor_count']):,} vs {int(da['visitor_count']):,} visitors).")
            ...
            st.markdown(f'<div class="insight-box" style="margin-top:16px;"><b>Comparative Interpretation:</b> {"  ".join(interps)}<br/><i style="font-size:11px;color:#0369a1;">Based on observable signals and directional proxies. Not causal attribution or guaranteed ROI forecast.</i></div>',unsafe_allow_html=True)
    else:
...
# ══════════════════════════════════════════════════════════════════
    ...
    IMPL={'중국':("Package + Korean Wave Campaign","High visitor volume and Korean Wave spending — bundle K-beauty and K-pop"),
          '일본':("Repeat Visitor Promotion","High revisit rate — loyalty programs and short-trip packages"),
          '대만':("SNS-Driven Influencer Campaign","Strong SNS engagement — influencer and UGC campaigns"),
          '미국':("Premium Long-Haul Package","Highest spend/person — target high-value travelers"),
    ...
    for i,ctry in enumerate(CNAME):
        ...
        with cols[i]:
            st.markdown(f'<div class="card" style="padding:20px 16px;border-top:4px solid {COLORS[i]};text-align:center;"><div style="margin-bottom:8px;">{flag(ctry,36)}</div><div style="font-family:Outfit,sans-serif;font-size:14px;font-weight:700;color:#0f172a;margin-bottom:14px;">{CNAME[ctry]}</div>{rows_html}<div style="margin-top:12px;padding:10px;background:#f8fafc;border-radius:8px;text-align:left;"><div style="font-size:10px;color:#94a3b8;letter-spacing:.08em;text-transform:uppercase;margin-bottom:4px;">Marketing Implication</div><div style="font-size:11px;font-weight:600;color:#4f46e5;margin-bottom:3px;">{it}</div><div style="font-size:11px;color:#64748b;line-height:1.4;">{id_}</div></div></div>',unsafe_allow_html=True)
    st.markdown("<br/>",unsafe_allow_html=True)
    ...
    with c4:
        ...
        fig=px.bar(lind,x='Market',y='ratio',color='industry_en',barmode='stack',color_discrete_sequence=['#4f46e5','#f59e0b','#10b981','#ef4444','#8b5cf6','#ec4899','#06b6d4','#84cc16','#f97316'],labels={'ratio':'Share (%)','Market':'','industry_en':'Category'})
        fig.update_layout(height=270,**CHART); st.plotly_chart(fig,use_container_width=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with c1: ctry_f=st.multiselect("Markets",list(CNAME.keys()),default=list(CNAME.keys()),format_func=lambda x:CNAME[x])
    with c2:
        MMAP={'Visitor Count':'visitor_count','Visitor MoM Growth (%)':'Visitor MoM Growth (%)','SNS Buzz':'buzz_volume','SNS Engagement':'engagement','Potential Exposure':'potential_exposure','Positive Sentiment (%)':'positive_pct','Korean Wave Transactions':'hallyu_spend_count','ML High Priority Probability':'ml_priority_score','Market Share (%)':'Market Share (%)'}
...
# ══════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        for k,v in [("Data Period","2018.11 – 2026.04"),("Unit of Analysis","Country × Month panel data"),
    ...
    with c2:
        ...
        for cat,feats in [("🧳 Visitor Metrics (10)","visitor_count, lag 1–3, MoM growth, 3M/6M moving avg, vs 3M avg, rolling std, country share"),
    ...
    st.markdown("---")
    st.markdown('<div class="lbl">Market Attractiveness Score — Design</div>',unsafe_allow_html=True)
    ...
    The Random Forest model predicts whether each market belongs to the High, Medium, or Low Priority class.
    Its <b>High Priority probability</b> is used as the ML component of the <b>Market Attractiveness Score</b>.<br/><br/>
    To make the output more useful for marketers, the final Market Attractiveness Score combines the ML probability
    component with observable business signals:<br/><br/>
    <b style="color:#4f46e5;">Market Attractiveness Score =</b><br/>
    &nbsp;&nbsp;45% × ML High Priority Probability (Random Forest)<br/>
    &nbsp;&nbsp;+ 20% × Demand Score (visitor volume, momentum, vs 3M avg)<br/>
    &nbsp;&nbsp;+ 15% × Digital Momentum Score (SNS engagement, buzz, growth rates)<br/>
    &nbsp;&nbsp;+ 10% × Sentiment Score (positive sentiment %)<br/>
    &nbsp;&nbsp;+ 10% × Revenue / Korean Wave Score (market value proxy, K-Wave spending)<br/><br/>
    <i>Market Attractiveness Score is not a subjective preference score or a guaranteed ROI forecast.
    It is an ML-informed decision score designed to support relative market comparison and scenario analysis.</i>
    ...
    <b style="color:#4f46e5;">Step 5 — Market Attractiveness Score:</b> At inference time, the Random Forest High Priority probability (45%) is combined with observable business signals (55%) to produce the Market Attractiveness Score.<br/>
    <b style="color:#4f46e5;">Step 6 — Scenario Calibration (What-If only):</b> A bounded post-processing calibration layer is applied for interactive simulation only. The calibration coefficients are intentionally capped and used only to improve simulation usability. They do not replace the trained model and do not determine the main market ranking. Not used for model training, test evaluation, feature importance, confusion matrix, or the main dashboard ranking.
    </div></div>""",unsafe_allow_html=True)
    st.markdown("---")
    st.markdown('<div class="lbl">User Interaction</div>',unsafe_allow_html=True)
    st.markdown("""<div style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 1px 3px rgba(0,0,0,.08);">
    <div style="font-size:13px;color:#475569;line-height:2.2;">
    Users can inspect ML-based market attractiveness rankings on the <b>Overview</b> page,
    adjust market indicators in the <b>Priority Engine</b> and receive real-time ML predictions with score component breakdown,
    simulate what-if scenarios via sliders with economically interpretable feedback on the <b>Scenario-Adjusted Market Attractiveness Score</b>,
    compare two markets head-to-head using <b>Compare Markets</b>,
    allocate marketing budgets proportionally to Market Attractiveness Scores using the <b>Budget Planner</b>,
    ...
    with c1:
        comp_data={'Model':['Rule-Based Baseline','Logistic Regression','Random Forest ✓'],
    ...
    st.markdown("""<div class="proxy-note" style="margin-top:16px;"><b>Revenue Proxy Note:</b> Revenue Opportunity Level and Market Value Proxy are directional business proxies, not causal ROI forecasts. They combine the Market Attractiveness Score components — visitor scale, average spending, ML priority signals, SNS attention, and Korean Wave spending — to support relative market comparison.</div>""",unsafe_allow_html=True)

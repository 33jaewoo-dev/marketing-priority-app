...
def load_model():
    ...
    feat_imp = pd.read_csv(os.path.join(base, 'feature_importance.csv'), encoding='utf-8-sig')
    return model, le, feature_cols, feat_imp

@st.cache_data
def load_data():
    base = os.path.dirname(os.path.abspath(__file__))
    df = pd.read_csv(os.path.join(base, 'full_dataset.csv'), encoding='utf-8-sig')
    sat = pd.read_csv(os.path.join(base, 'satisfaction_data.csv'), encoding='utf-8-sig')
    hallyu = pd.read_csv(os.path.join(base, 'hallyu_spending.csv'), encoding='utf-8-sig')
    hallyu_ind = pd.read_csv(os.path.join(base, 'hallyu_industry.csv'), encoding='utf-8-sig')
...
if page == "📊 Dashboard":
    ...
    st.subheader(f"📅 Latest Priority Ranking — {latest_month[:4]}.{latest_month[4:]}")
    cols = st.columns(5)

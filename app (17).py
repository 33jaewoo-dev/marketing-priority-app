...
def calc_score(inp, df_ref, ml_label=None):
    ...
    Each variable follows economically grounded directional logic:
    - Visitor volume/momentum ↑ → score ↑
    - SNS engagement/buzz/exposure ↑ → score ↑
    - Positive sentiment ↑ → score ↑ (linear, always responsive)
    - Market share ↑ → score ↑
    - Hallyu spending ↑ → score ↑
    - Visitor crash (MoM < -15%) → penalty
    - Severe negative sentiment (< 50%) → penalty
    - High oil price (> 70 USD) → penalty (aviation cost rises)
    - Weak KRW (high exchange rate) → bonus (Korea cheaper for foreigners)
    - Peak season (months 3,4,5,9,10) → bonus
    - Visitor + SNS growth synergy → bonus
    - ML Adjustment: RF classification corrects rule score when they disagree
    """
    # Factor 1: Visitor Volume (0-20pts)
    v = inp['visitor_count']
    if   v > 500000: f_vol = 20
    elif v > 300000: f_vol = 17
    elif v > 200000: f_vol = 14
    elif v > 100000: f_vol = 11
    elif v >  50000: f_vol = 8
    elif v >  20000: f_vol = 5
    else:            f_vol = 2

    # Factor 2: Visitor Momentum (0-20pts)
    mom = inp['visitor_mom_growth']
    if   mom >  0.30: f_mom = 20
    elif mom >  0.20: f_mom = 17
    elif mom >  0.10: f_mom = 14
    elif mom >  0.05: f_mom = 11
    elif mom >  0.00: f_mom = 8
    elif mom > -0.05: f_mom = 6
    elif mom > -0.15: f_mom = 4
    elif mom > -0.30: f_mom = 2
    else:             f_mom = 0
    vvs3 = inp['visitor_vs_3m_avg']
    if   vvs3 > 1.30: f_mom = min(20, f_mom + 3)
    elif vvs3 > 1.15: f_mom = min(20, f_mom + 2)
    elif vvs3 > 1.05: f_mom = min(20, f_mom + 1)

    # Factor 3: SNS Engagement Volume (0-15pts) — primary interest signal
    e = inp['engagement']
    if   e > 3000000: f_eng = 15
    elif e > 1000000: f_eng = 13
    elif e >  500000: f_eng = 11
    elif e >  200000: f_eng = 9
    elif e >   50000: f_eng = 6
    elif e >   10000: f_eng = 3
    else:             f_eng = 1

    # Factor 4: SNS Buzz Volume (0-8pts)
    b = inp['buzz_volume']
    if   b > 300000: f_buzz = 8
    elif b > 100000: f_buzz = 7
    elif b >  50000: f_buzz = 6
    elif b >  30000: f_buzz = 5
    elif b >  10000: f_buzz = 3
    elif b >   3000: f_buzz = 2
    else:            f_buzz = 1

    # Factor 5: Potential Exposure (0-5pts) — reachable audience size
    pe = inp['potential_exposure']
    if   pe > 5000000: f_exp = 5
    elif pe > 2000000: f_exp = 4
    elif pe > 1000000: f_exp = 3
    elif pe >  500000: f_exp = 2
    elif pe >  100000: f_exp = 1
    else:              f_exp = 0

    # Factor 6: SNS Momentum (0-10pts)
    emom = inp['engagement_mom_growth']
    bmom = inp['buzz_mom_growth']
    sns_mom = emom * 0.65 + bmom * 0.35
    if   sns_mom >  0.30: f_sns_g = 10
    elif sns_mom >  0.20: f_sns_g = 8
    elif sns_mom >  0.10: f_sns_g = 7
    elif sns_mom >  0.00: f_sns_g = 5
    elif sns_mom > -0.10: f_sns_g = 3
    elif sns_mom > -0.20: f_sns_g = 2
    else:                 f_sns_g = 0

    # Factor 7: Positive Sentiment (0-10pts, fully linear)
    f_sent = inp['positive_pct'] * 0.10

    # Factor 8: Market Share (0-5pts)
    cs = inp['country_share']
    if   cs > 0.40: f_share = 5
    elif cs > 0.30: f_share = 4
    elif cs > 0.20: f_share = 3
    elif cs > 0.10: f_share = 2
    else:           f_share = 1

    # Factor 9: Hallyu Spending (0-7pts)
    h = inp['hallyu_spend_count']
    if   h > 1500000: f_hal = 7
    elif h > 1000000: f_hal = 6
    elif h >  500000: f_hal = 5
    elif h >  200000: f_hal = 3
    elif h >   50000: f_hal = 2
    else:             f_hal = 0

    base = f_vol + f_mom + f_eng + f_buzz + f_exp + f_sns_g + f_sent + f_share + f_hal

    # Penalty 1: Visitor crash (external shock — THAAD, COVID)
    vmom_val = inp['visitor_mom_growth']
    p_shock = (-25 if vmom_val < -0.50 else -15 if vmom_val < -0.30 else -7 if vmom_val < -0.15 else 0)

    # Penalty 2: Negative public sentiment
    pos = inp['positive_pct']
    p_sent = (-15 if pos < 40 else -7 if pos < 50 else 0)

    # Penalty 3: High oil price → higher airfare → Korea less attractive
    oil = inp['oil_price']
    p_oil = (-5 if oil > 120 else -3 if oil > 90 else -1 if oil > 70 else 0)

    # Bonus 1: Weak KRW → Korea cheaper for foreign visitors
    exr = inp['exchange_rate']
    exr_p75 = float(df_ref['exchange_rate'].quantile(0.75))
    exr_p50 = float(df_ref['exchange_rate'].quantile(0.50))
    b_fx = (4 if exr > exr_p75 else 2 if exr > exr_p50 else 0)

    # Bonus 2: Peak season (spring/fall travel peaks)
    b_peak = 4 if inp['is_peak_season'] else 0

    # Bonus 3: Growth synergy — visitor + SNS both rising → max ROI window
    b_synergy = (3 if mom > 0.10 and sns_mom > 0.10 else
                 1 if mom > 0.05 and sns_mom > 0.00 else 0)

    score = base + p_shock + p_sent + p_oil + b_fx + b_peak + b_synergy

    # ML Hybrid Adjustment
    ml_adj = 0
    ml_note = ""
    if ml_label is not None:
        if   ml_label == 'High'   and score < 50:
            ml_adj, ml_note = +10, "▲ ML upward correction: strong multi-factor interaction detected"
        elif ml_label == 'High'   and score < 65:
            ml_adj, ml_note = +5,  "▲ ML upward correction: above-average composite signal"
        elif ml_label == 'Low'    and score > 65:
            ml_adj, ml_note = -10, "▼ ML downward correction: hidden risk pattern detected"
        elif ml_label == 'Low'    and score > 50:
            ml_adj, ml_note = -5,  "▼ ML downward correction: below-average composite signal"
        elif ml_label == 'Medium' and score > 78:
            ml_adj, ml_note = -5,  "▼ ML correction: mixed signals temper high rule score"
        elif ml_label == 'Medium' and score < 35:
            ml_adj, ml_note = +5,  "▲ ML correction: partial positive signals lift low rule score"
    ...
    breakdown = {
        ...
        'Sentiment':         round(f_sent, 1),
        'Market Share':      f_share,
        'Hallyu Spending':   f_hal,
        'Visitor Crash':     p_shock,
        'Sentiment Penalty': p_sent,
        'Oil Price Penalty': p_oil,
        'Weak KRW Bonus':    b_fx,
        'Peak Season Bonus': b_peak,
        'Growth Synergy':    b_synergy,
